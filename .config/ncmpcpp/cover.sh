...

{
    ...
    if [[ -n "$src" ]] ; then
        ...
        convert "$src" -resize 450x -background "#2F343F" -extent 1000x500 "$COVER"
        if [[ -f "$COVER" ]] ; then

          #printf "\e]20;${COVER};75x75+0+5:op=keep-aspect\a"
    ...
    if [[ -n "$src" ]] ; then
        ...
        if [[ -f "$COVER" ]] ; then
            ...
            dunstify -u low -i ${COVER} "  Now Playing:" "`mpc current`"
        fi
    fi
} &
